...
object SkinManager {
    ...
    fun switchTheme(theme: Int, ctx: Context?) {
        if (ctx == null) {
            loaderServer.forEach {
                val context = it.ctxRef.get()
                if (context != null) {
                    InflaterInterceptor.switchTheme(providerFactory.getResourceProvider(context, theme))
                }
            }
        } else {
            InflaterInterceptor.switchTheme(providerFactory.getResourceProvider(ctx, theme))
        }
    }
}
